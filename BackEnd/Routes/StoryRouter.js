...
module.exports = (io) => {
  ...
  router.put("/:id", protectRoute, async (req, res) => {
    try {
      ...
      console.error("Error updating story:", error.message, error.stack);
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  router.delete("/:id", protectRoute, async (req, res) => {
    try {
      ...
      console.error("Error deleting story:", error.message, error.stack);
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  router.get("/", protectRoute, async (req, res) => {
    try {
      ...
      console.error("Error fetching stories:", error.message, error.stack);
      res.status(500).json({ message: "Server error" });
    }
  });
  ...
  router.post("/:storyId/view", protectRoute, async (req, res) => {
    try {
      ...
      if (!story.viewers.includes(req.user._id)) { // Changed from req.user.id
      ...
      console.error("Error viewing story:", error.message, error.stack);
      res.status(500).json({ message: "Server error" });
    }
  });
