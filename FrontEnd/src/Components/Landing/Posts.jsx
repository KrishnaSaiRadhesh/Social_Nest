...
//   try {
//     // Update backend
//     await axios.post(`http://localhost:3000/api/posts/${postId}/like`, {}, { withCredentials: true });
...
//                       <div className='Like cursor-pointer' onClick={() => handleLike(post._id)}>
//                           <IoMdHeartEmpty className={likes[post._id] ? 'text-red-500' : ''} />
//                           <h5>{likes[post._id] ? 'Liked' : 'Like'}</h5>
                       
//                         </div>
//                         <p>"length"{post.likes?.length}</p>
...
//   const [posts, setPosts] = useState([]);
//   const [loading, setLoading] = useState(false);
//   const [likes, setLikes] = useState({}); // { postId: true/false }
...
//       // Update posts state with new like count and status
//       setPosts((prevPosts) =>
//         prevPosts.map((post) =>
//           post._id === postId
//             ? { ...post, likes: new Array(res.data.likes).fill(null), liked: res.data.liked } // Use count for length
//             : post
//         )
//       );

//       // Update likes state for UI
...
//         // Initialize likes state based on whether the user liked each post
//         const initialLikes = {};
//         res.data.forEach((post) => {
//           initialLikes[post._id] = post.liked || false; // Assuming backend returns `liked`
//         });
...
const Posts = () => {
  ...
  const [comments, setComments] = useState({}); // { postId: [ { user, text, createdAt } ] }
