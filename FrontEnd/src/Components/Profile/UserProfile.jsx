...
// import axios from "axios";
// import { useNavigate, useParams } from "react-router-dom";
// import { BsImage, BsCameraVideo, BsArrowLeft } from "react-icons/bs";
...
//         const profileRes = await axios.get(
//           `https://social-nest-2.onrender.com/api/user/${id}`, // Changed from /api/user/:id
//           {
//             withCredentials: true,
//           }
//         );
...
//         const currentUserRes = await axios.get(
//           "https://social-nest-2.onrender.com/api/user", // Changed from /api/user/profile
//           {
//             withCredentials: true,
//           }
//         );
...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const profileRes = await api.get(`/user/${id}`); // Use api
        const profileData = profileRes.data;
        ...
        const currentUserRes = await api.get("/user/user"); // Use api
        const currentUserData = currentUserRes.data;
        setCurrentUserId(currentUserData._id);
        setIsFollowing(currentUserData.following?.includes(id) || false);
      } catch (error) {
  ...
  const handleFollow = async () => {
    try {
      await api.post(`/user/follow/${id}`); // Use api
      setIsFollowing(true);
      setFollowers((prev) => [...prev, currentUserId]);
    } catch (error) {
      console.error("Error following user:", error);
  ...
  const handleUnFollow = async () => {
    try {
      await api.post(`/user/unfollow/${id}`); // Use api
      setIsFollowing(false);
      setFollowers((prev) =>
        prev.filter((followerId) => followerId !== currentUserId)
      );
