...
const ChatApp = ({ socket }) => {
  ...
  useEffect(() => {
    ...
    const fetchFriend = async () => {
      if (friendId) {
        try {
          const res = await axios.get(`http://localhost:3000/api/user/${friendId}`, {
            withCredentials: true,
          });
          setFriend(res.data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (friendId) {
        try {
          const res = await axios.get(`http://localhost:3000/api/messages/${friendId}`, {
            withCredentials: true,
          });
        ...
        {friendId ? (
            ...
            <div className="p-4 border-b flex items-center gap-3 bg-gray-50">
              ...
              {friend && (
                <div className="flex items-center gap-3">
                  ...
                  />
                  <h1 className="text-xl font-semibold text-gray-800">{friend.name}</h1>
                </div>
              )}
            </div>
              ...
              {messages.length > 0 ? (
                messages.map((msg) => (
                  <div
                    key={msg._id}
                    className={`mb-4 flex ${
                      msg.senderId === friendId ? 'justify-start' : 'justify-end'
                    }`}
                  >
                    <div
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-semibold text-gray-600">Chat with Friends</h2>
